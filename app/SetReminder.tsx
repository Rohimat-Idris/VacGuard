...
const ReminderSection: React.FC<ChildrenListProps> = ({ parentId }) => {
  ...
  const frequencyOptions = ["Once", "Daily", "Weekly"];
  ...
  const saveReminder = async () => {
    if (!selectedChild || !selectedVaccine || !frequency) {
    ...
    const reminderData = {
      ...
      alarm_date: date.toISOString().split('T')[0],
      alarm_time: date.toTimeString().split(' ')[0],
      frequent_time: frequency,
      personal_note: "Personal note here...",
    };
  
    try {
      const response = await fetch('http://192.168.0.2/vaxkids/children/reminders.php', {
        method: 'POST',
        headers: {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.header}>Set a Friendly Reminder!</Text>
      <Text style={styles.description}>Never miss an important vaccine day for your little one!</Text>
      ...
      >
        ...
        <View style={styles.field}>
          ...
          <Picker
            ...
            onValueChange={(itemValue) => setFrequency(itemValue as string)}
            style={styles.picker}
          >
        ...
        <View style={styles.field}>
          ...
          <TextInput placeholder="Add a personal note here..." style={styles.messageInput} />
        </View>
